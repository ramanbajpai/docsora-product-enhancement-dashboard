...
interface WatermarkSuccessProps {
  files: File[];
  pageCount?: number;
  onReset: () => void;
}
...
export function WatermarkSuccess({ files, pageCount = 1, onReset }: WatermarkSuccessProps) {
  const fileName = files[0]?.name || "document.pdf";
  const outputName = fileName.replace(".pdf", "_watermarked.pdf");
    ...
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          <span className="text-primary font-semibold">
            Watermark applied to <AnimatedCounter value={pageCount} suffix={pageCount === 1 ? " page" : " pages"} delay={1200} />
          </span>
        </motion.p>
      </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — watermark with images & logos
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Image watermarks • Batch apply • Custom positioning
                  </p>
                </div>
              </div>
      ...
      >
        <p className="text-[9px] text-muted-foreground/30 text-center">
          Files are processed securely and removed automatically after watermarking.
        </p>
      </motion.div>
    </div>
  );
