...
export function CompareSuccess({ files, onReset }: CompareSuccessProps) {
  const fileName = files.length >= 2 
    ? `${files[0]?.name.replace(/\.[^/.]+$/, '')}_vs_${files[1]?.name.replace(/\.[^/.]+$/, '')}_summary.pdf`
    : 'comparison_summary.pdf';

  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4 min-h-[calc(100vh-8rem)] flex flex-col justify-center">
      {/* Completion Hero */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        >
          ...
          <span className="relative z-10">Download Comparison Report</span>
        </motion.button>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — advanced comparison tools
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Batch comparisons • Detailed change tracking • AI-powered insights
                  </p>
                </div>
              </div>
      ...
      >
        Files are processed securely and removed automatically after comparison.
      </motion.p>
    </div>
  );
}