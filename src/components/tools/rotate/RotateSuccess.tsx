...
import { 
  ...
  Scissors, Lock, RotateCw, ArrowUpDown
} from "lucide-react";
import { cn } from "@/lib/utils";

interface RotateSuccessProps {
  file: File;
  onReset: () => void;
}
...
export function RotateSuccess({ file, onReset }: RotateSuccessProps) {
  const navigate = useNavigate();

  const handleDownload = () => {
    // In real implementation, would download the rotated file
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — batch rotate & advanced tools
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Rotate multiple files • Auto-correct orientation • No limits
                  </p>
                </div>
              </div>
