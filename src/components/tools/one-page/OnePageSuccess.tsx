...
export function OnePageSuccess({ files, originalPageCount = 5, onReset }: OnePageSuccessProps) {
  const fileName = files[0]?.name || "document.pdf";
  const outputName = fileName.replace(".pdf", "_onepage.pdf");
    ...
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          <span className="text-foreground font-medium">
            <AnimatedCounter value={originalPageCount} suffix={originalPageCount === 1 ? " page" : " pages"} delay={1200} />
          </span>
          <span className="mx-2 text-muted-foreground/30">→</span>
          <span className="text-primary font-semibold">1 continuous page</span>
        </motion.p>
      </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — convert with custom layouts
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Batch convert • Custom sizing • Layout presets
                  </p>
                </div>
              </div>
      ...
      >
        <p className="text-[9px] text-muted-foreground/30 text-center">
          Files are processed securely and removed automatically after conversion.
        </p>
      </motion.div>
    </div>
  );
