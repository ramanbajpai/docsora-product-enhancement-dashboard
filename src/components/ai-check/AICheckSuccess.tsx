...
import { Download, Languages, PenTool, Send, HardDrive, ArrowRight, Sparkles, Zap, Minimize2, FileText, SplitSquareHorizontal, Droplets, Edit3, Database, WrenchIcon, GitCompare, FolderArchive, RefreshCw, ChevronDown, RotateCw, Trash2, Shield, FileOutput, Layers, Grid3X3 } from "lucide-react";
import { cn } from "@/lib/utils";
import { useEffect, useState, useRef, useCallback } from "react";
import { toast } from "sonner";

interface AICheckSuccessProps {
  fileName: string;
  onReset: () => void;
}
...
function ContinueWorkflowSection() {
  ...
  return (
    ...
    >
      ...
      {/* Expanded tools panel (below row, not floating) */}
      <AnimatePresence>
        {toolsOpen && (
          <motion.div
...
export function AICheckSuccess({ fileName, onReset }: AICheckSuccessProps) {
  const [downloadStarted, setDownloadStarted] = useState(false);

  const triggerDownload = useCallback(() => {
    // In a real app, this would download the actual enhanced file
  ...
  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          <span className="text-foreground font-medium">
            <AnimatedCounter value={7} delay={1000} />
          </span>
          {" issues fixed"}
          <span className="mx-2.5 text-muted-foreground/30">•</span>
          <span className="text-foreground font-medium">
            <AnimatedCounter value={12} delay={1200} />
          </span>
          {" enhancements"}
          <span className="mx-2.5 text-muted-foreground/30">•</span>
          {"Quality "}
          <span className="text-primary font-semibold">
            <AnimatedCounter value={92} suffix="%" delay={1400} />
          </span>
        </motion.p>
      </motion.div>
      ...
      {/* Upgrade to Pro - Premium Glassmorphic Banner */}
      <motion.div
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — make every document exceptional
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Deeper rewrites • Tone control • Unlimited checks
                  </p>
                </div>
              </div>
      ...
      >
        <p className="text-[9px] text-muted-foreground/30 flex items-center justify-center gap-1">
          ...
          Enterprise-grade AI. No data stored after processing.
        </p>
      </motion.div>
    </div>
  );
