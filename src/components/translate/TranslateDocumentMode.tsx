...
import { Upload, X, ArrowRight, CheckCircle2, ChevronDown } from "lucide-react";
...
const supportedFormats = ['PDF', 'DOCX', 'DOC', 'TXT', 'PPTX', 'PPT'];

interface TranslateDocumentModeProps {
...
export const TranslateDocumentMode = ({
  onTranslate,
  onStartTranslating,
}: TranslateDocumentModeProps) => {
  ...
  const processFile = (selectedFile: File) => {
    ...
    if (!validTypes.includes(selectedFile.type)) {
      toast.error("Please upload a PDF, DOCX, TXT, or PPTX file");
      return;
    }
        ...
        >
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-muted-foreground">Formatting preserved</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
              <span className="text-muted-foreground">Layout safe</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
              <span className="text-muted-foreground">Ready to translate</span>
            </div>
          </div>
