...
interface TranslateSuccessProps {
...
function CompletionAnimation() {
  return (
    ...
    >
      {/* Glow pulse - animates once then settles to subtle */}
      <motion.div
...
export const TranslateSuccess = ({ result, onReset }: TranslateSuccessProps) => {
  ...
  const getSubtext = () => {
    if (isTextMode) {
      return "Your text has been translated successfully.";
    }
    
    const isDualMode = result.mode === "dual";
    const separator = isDualMode ? "+" : "→";
    
    return (
      <>
        <span>{languageNames[result.sourceLanguage] || result.sourceLanguage}</span>
        <span className="mx-2 text-muted-foreground/30">{separator}</span>
        <span className="text-primary font-medium">{languageNames[result.targetLanguage] || result.targetLanguage}</span>
      </>
    );
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          {isTextMode ? "Your translation is ready" : "Your translated document is ready"}
        </motion.h1>
        
        {/* Metrics as flowing text */}
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      >
        <motion.button
          onClick={isTextMode ? handleCopy : handleDownload}
        ...
        <button
          onClick={onReset}
          className="block mx-auto text-[11px] text-muted-foreground/50 hover:text-muted-foreground transition-colors mt-3"
        >
          Translate another
        </button>
      </motion.div>
        ...
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="relative px-6 py-5 backdrop-blur-xl">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — translate more with extra features
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Batch translation • 50+ languages • No file limits
                  </p>
                </div>
              </div>
              ...
              >
                <motion.div
                  className="absolute inset-0 bg-gradient-to-r from-transparent via-primary/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500"
                />
      ...
      >
        <p className="text-[10px] text-muted-foreground/30">
          Your documents are processed securely and never stored after download.
        </p>
      </motion.div>
    </div>
  );
