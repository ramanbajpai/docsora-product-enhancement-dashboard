...
const ConvertResult = ({ conversionData, fileName, fileCount = 1, onReset }: ConvertResultProps) => {
  const [downloadStarted, setDownloadStarted] = useState(false);

  const getNewFileName = () => {
    const baseName = fileName.replace(/\.[^/.]+$/, "");
    return `${baseName}.${conversionData.targetFormat.toLowerCase()}`;
  };
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      triggerDownload();
    }, 1500); // Delay to let animation complete
  ...
  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          Converted to {conversionData.targetFormat} · {fileCount} {fileCount === 1 ? "file" : "files"}
        </motion.p>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <span className="relative z-10">
            {fileCount > 1 ? "Download Files" : "Download Document"}
          </span>
        </motion.button>
        
        <p className="text-[10px] text-muted-foreground/40 mt-3 truncate max-w-[220px] mx-auto">
          {fileCount > 1 ? `${fileCount} files` : getNewFileName()}
        </p>
        <button
          onClick={onReset}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — convert without limits
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Batch conversions · Priority processing · No file limits
                  </p>
                </div>
              </div>
      ...
      >
        Files are processed securely. No data is stored after conversion.
      </motion.p>
    </div>
  );
};
