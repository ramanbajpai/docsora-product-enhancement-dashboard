...
export const CompressResult = ({ file, result, onReset }: CompressResultProps) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        {/* Metrics as flowing text - matches AI Check style */}
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <span className="text-foreground font-medium">{formatSize(result.originalSize)}</span>
          <span className="mx-2 text-muted-foreground/30">→</span>
          <span className="text-foreground font-medium">{formatSize(result.compressedSize)}</span>
          <span className="mx-2.5 text-muted-foreground/30">•</span>
          <span className="text-primary font-semibold">
            <AnimatedCounter value={result.reduction} suffix="% smaller" delay={1200} />
          </span>
        </motion.p>
      </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — compress larger files with more control
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Batch processing • Custom quality • No file limits
                  </p>
                </div>
              </div>
      ...
      >
        <p className="text-[9px] text-muted-foreground/30 text-center">
          Files are processed securely and removed automatically after compression.
        </p>
      </motion.div>
    </div>
  );
